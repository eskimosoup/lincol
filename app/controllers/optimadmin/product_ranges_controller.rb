module Optimadmin
  class ProductRangesController < Optimadmin::ApplicationController
    ...
    def index
      @product_ranges = Optimadmin::BaseCollectionPresenter.new(collection: ProductRange.industry_filter(params[:industry_id]).positioned.where('title ILIKE ?', "#{params[:search]}%").order(:industry_id).page(params[:page]).per(params[:per_page] || 15), view_template: view_context, presenter: Optimadmin::ProductRangePresenter)
    end
