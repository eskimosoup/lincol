module Optimadmin
  class FrequentlyAskedQuestionsController < Optimadmin::ApplicationController
    ...
    def index
      @frequently_asked_questions = Optimadmin::BaseCollectionPresenter.new(collection: FrequentlyAskedQuestion.where('question ILIKE ?', "#{params[:search]}%").industry_filter(params[:industry_id]).ordered.page(params[:page]).per(params[:per_page] || 15), view_template: view_context, presenter: Optimadmin::FrequentlyAskedQuestionPresenter)
    end
  ...
  private
    ...
    def frequently_asked_question_params
      params.require(:frequently_asked_question).permit(:question, :answer, :position, :industry_id, :display)
    end
  end
end
